...
join juror_mod.pool p on bs.pool_number = p.pool_no and p.owner <> '400'
...
select coalesce ((select s.no_requested from snapshot_data s),coalesce(c.no_requested,0),0) as no_requested,
		...
		c.postponed::integer,
		(c.non_responded + coalesce((select s.non_responded from snapshot_data s),0) - c.awaiting_info - coalesce((select s.awaiting_info from snapshot_data s),0))::integer as non_responded,
		c.undelivered::integer,
		(c.unavailable + coalesce((select s.non_responded from snapshot_data s), 0) + c.awaiting_info - c.undelivered)::integer as unavailable,
		...
		(c.awaiting_info + coalesce((select s.awaiting_info from snapshot_data s),0))::integer,
