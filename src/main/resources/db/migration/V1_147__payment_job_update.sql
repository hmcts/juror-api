alter table juror_mod.payment_data
...
BEGIN
    ...
    FOR payments IN
        ...
        FROM (SELECT DATE_TRUNC('day', pd.creation_date) as creation_date,
              ...
              WHERE DATE_TRUNC('day', pd.creation_date) <=
                    DATE_TRUNC('day', CURRENT_TIMESTAMP) -- need to check this date range as
...
BEGIN
    ...
    FOR c_extract IN
        SELECT pd.loc_code,
               ...
               coalesce(REPLACE(REPLACE(REPLACE(pd.bank_ac_name, '|', ' '), chr(10), ' '), chr(13), ' '),
                        '')                                                                                 bank_ac_name,
