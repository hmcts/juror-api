...
begin
    ...
    return query select
                     ...
                     least(min(coalesce(case
								when jp.status = 10 then jp.transfer_date
								when jp.status = 8 then jp.reassign_date
								else date(j.completion_date) end, p_end_date)),p_end_date) service_end,
                 ...
                 where p.loc_code = a.loc_code
                   and (((jp.status in (2,3,4,10,13) or (jp.status = 7 and a.attendance_date is not null and a.attendance_date between p_start_date and p_end_date)) and jp.is_active = true)
                     or (jp.status = 8 and a.attendance_date is not null and jp.reassign_date is not null))
                   ...
                   and ((jp.status = 10 and jp.transfer_date >= p_start_date)
					 or (jp.status = 8 and jp.reassign_date >= p_start_date)
