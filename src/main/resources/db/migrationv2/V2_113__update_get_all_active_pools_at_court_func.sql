-- JS-966 clarify the where clause jt.loc_code = p.loc_code and (jt.result = 'J' or jt.result is null)
-- with brackets and the and condition outside the brackets to ensure that the loc_code is always checked for jurors on trial

DROP FUNCTION juror_mod.get_all_active_pools_at_court_location(text);

CREATE OR REPLACE FUNCTION juror_mod.get_all_active_pools_at_court_location(p_loccode text)
 RETURNS TABLE(pool_number character varying, total_possibly_in_attendance bigint, in_attendance bigint, on_call bigint, total_possibly_on_trial bigint, jurors_on_trial bigint, pool_type character varying, service_start_date date)
 LANGUAGE plpgsql
AS $function$
begin
    ...
    return query with cte_first_query as (
                                  select
                                  	...
                                  	sum(case when jp.status = 2 and (jp.on_call is null or jp.on_call = false) then 1 else 0 end) as total_possibly_in_attendance,
                                  	sum(case when jp.status in (3, 4) then 1 else 0 end) as total_possibly_on_trial,
                                  	p.pool_type,
                                  ...
                                  select
                                  	...
                                  	sum(case when jp.status in (3, 4) and jp.juror_number in (select juror_number from juror_mod.juror_trial jt where jt.loc_code = p.loc_code and (jt.result = 'J' or jt.result is null)) then 1 else 0 end) as jurors_on_trial
                                  from
                                  	cte_first_query p
                                  ...
                                  on
                                  	...
                                  	and jp.pool_number = a.pool_number
                                  where
                                  ...
                                  select
                                  	cte1.pool_number,
                                  	cte1.total_possibly_in_attendance,
                                  	cte2.in_attendance,
                                  	cte1.on_call,
                                  	cte1.total_possibly_on_trial,
...
-- also update the related function to clarify the brackets to ensure that the loc_code is always checked for jurors on trial
