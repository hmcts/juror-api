	...
	BEGIN
	    ...
	    LOOP
	        ...
	        IF (v_print_msg IS NULL) THEN
	   			CALL juror_mod.hk_insert_audit(temprow.juror_number, v_start_time, 'Deleted Juror');
    ...
    FOR temprow IN 
		...
		*/
        ...
        LOOP
	        ...
	        -- log if the deletion was successful or not based on the return value of the call
	        IF (v_print_msg IS NULL) THEN
	   			CALL juror_mod.hk_insert_audit(temprow.juror_number, v_start_time, 'Deleted '||temprow.reply_type||' Response');
				v_rows_deleted := v_rows_deleted + 1;
			ELSIF POSITION('TIMED' IN v_print_msg) THEN
				EXIT;
			ELSE
