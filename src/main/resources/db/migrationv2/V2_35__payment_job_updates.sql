-- update to payment file function to produce a single file for all payments that are due rather than just today's date
...
BEGIN
        ...
        select SUM(pd.total)  INTO  v_total
        FROM (SELECT DATE_TRUNC('day', pd.creation_date) as creation_date,
              ...
              WHERE DATE_TRUNC('day', pd.creation_date) <= DATE_TRUNC('day', CURRENT_TIMESTAMP) -- need to check this date range as
                AND pd.expense_file_name IS NULL) pd;

        -- could we have a negative total and should we stop it?
        if (v_total is null or v_total <= 0.0 ) then return; end if;

        CALL juror_mod.payment_files_to_clob_write_to_clob(current_date, v_total, v_file_name);

        UPDATE juror_mod.payment_data
        SET expense_file_name = v_file_name
        WHERE DATE_TRUNC('day', creation_date) <= DATE_TRUNC('day', CURRENT_TIMESTAMP)
          and expense_file_name IS NULL;
...
-- function updated to produce a single file for all payments that are due rather than just today's date
    ...
    FOR c_extract IN
        SELECT pd.loc_code,
               ...
               UPPER(coalesce(REPLACE(REPLACE(REPLACE(pd.postcode, '|', ' '), chr(10), ' '), chr(13), ' '),
                              ''))                                                                                 postcode,
               pd.auth_code,
               UPPER(trim(coalesce(REPLACE(REPLACE(REPLACE(pd.juror_name, '|', ' '), chr(10), ' '), chr(13), ' '), ''))) name,
