...
AS SELECT b.id,
   ...
   FROM juror_mod.bulk_print_data b,
    ...
    juror_mod.pool p
  WHERE b.form_type::text = n.form_type::text AND b.juror_no::text = j.juror_number::text AND jp.juror_number::text = j.juror_number::text AND jp.pool_number::text = p.pool_no::text AND b.creation_date > (CURRENT_DATE - 3) AND b.digital_comms = false AND n.notification_type = 1 AND jp.owner::text = '400'::text AND jp.is_active = true AND j.h_email IS NOT NULL AND length(TRIM(BOTH FROM j.h_email)) > 0;

-- View Triggers

create trigger instead_of_update_bulkprintdatanotifycommsview instead of
update
    on
    juror_mod.bulk_print_data_notify_comms for each row execute function juror_mod.update_bulk_print_data();
