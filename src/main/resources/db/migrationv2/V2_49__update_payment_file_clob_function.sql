-- Updating procedure to replace special characters ' and # with space and '+' with '&' respectively
-- Also changing the file name format to JUROR_DDMMMYYNNNNNNNNNN.dat where NNNNNNNNNN is the sequence number
-- Jira story JS-19
...
BEGIN
    ...
    SELECT 'JUROR_' || TO_CHAR(p_creation_date, 'DD') || left(TO_CHAR(p_creation_date, 'Month'),3) || TO_CHAR(current_date, 'YY') || LPAD(v_revision::VARCHAR(20), 10, '0') || '.dat',
    ...
    FOR c_extract IN
        SELECT pd.loc_code,
               ...
               UPPER(coalesce(TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(pd.bank_ac_name, '|', ' '), '#', ' '), chr(39), ' '), chr(96), ' '), '+', '&'), chr(10), ' '), chr(13), ' ')),
                              ''))                                                                                 bank_ac_name,
