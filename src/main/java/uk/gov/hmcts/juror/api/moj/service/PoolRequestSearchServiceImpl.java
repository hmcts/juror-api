...
public class PoolRequestSearchServiceImpl implements PoolRequestSearchService {
    ...
    private void evaluateAdvancedSearchCriteria(JPAQuery<Tuple> query, PoolSearchRequestDto poolSearchRequestDto) {
        ...
        String nilPool = poolSearchRequestDto.getNilPool();
        ...
        if (nilPool != null) {
            log.debug(String.format("Nil Pool supplied as Pool Request Search parameter: %s", nilPool));
            poolRequestRepository.addNilPoolPredicate(query, nilPool);
        }
